...
# Distribute and configure aerospike.conf for each VM
...
EOF
  ...
  gcloud compute scp aerospike.conf $INSTANCE_NAME:/etc/aerospike/aerospike.conf --zone=europe-west1-b

done < $IP_FILE

# Check if the features.conf file exists
FEATURES_CONF_LOCAL_PATH="/etc/aerospike/features.conf"
if [ ! -f "$FEATURES_CONF_LOCAL_PATH" ]; then
  echo "features.conf file not found at $FEATURES_CONF_LOCAL_PATH"
  exit 1
fi

# Distribute features.conf to each VM
while read -r line; do
  INSTANCE_NAME=$(echo $line | awk '{print $1}')

  echo "Copying features.conf to $INSTANCE_NAME"
  gcloud compute scp "$FEATURES_CONF_LOCAL_PATH" $INSTANCE_NAME:/etc/aerospike/features.conf --zone=europe-west1-b

  echo "Restarting Aerospike on $INSTANCE_NAME"
  gcloud compute ssh $INSTANCE_NAME --zone=europe-west1-b --command "sudo systemctl restart aerospike"

done < $IP_FILE

echo "Configuration of Aerospike complete."
